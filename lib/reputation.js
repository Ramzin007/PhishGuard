...
const reputation = {
    ...
    /**
     * Simulates a domain age lookup (WHOIS).
     * @param {string} hostname 
     * @returns {Object} { ageInDays, riskScore, label }
     */
    getDomainAge: function(hostname) {
        // Mock data for demo
        const agedDomains = {
            'google.com': 9800,
            'amazon.com': 10500,
            'facebook.com': 7500,
            'paypal.com': 8800,
            'amaz0n.com': 2, // Very young!
            'secure-login-paypa1.com': 1,
            'localhost': 3650,
            '': 3650 // For local files, assume old/safe for age check
        };

        const age = agedDomains[hostname] !== undefined ? agedDomains[hostname] : 450; // Default to ~1.5 years for unknown but "safe" looking sites
        ...
        if (age < 30) {
            ...
            risk = 0.3;
            label = "Middle-aged (< 1y)";
        } else {
            label = "Established";
        }

        return { ageInDays: age, riskScore: risk, label: label };
    }
};
